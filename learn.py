...
def __train(train_image, train_label, model, batch_size=10, max_steps=200):
...
def infer(model_path, num_classes, orig):
    with tf.variable_scope('ejs') as scope:
        reuse = True
        (images_placeholder, keep_prob, logits) = __logits(num_classes)
...
if __name__ == '__main__':
    # train('model', 2, [])
    xs = []
    for x in Path(sys.argv[1]).glob('*.jpg'):
        xs.append((x.as_posix(), 0))
    for x in Path(sys.argv[2]).glob('*.jpg'):
        xs.append((x.as_posix(), 1))
    print(xs)
    train('model', 2, xs)
